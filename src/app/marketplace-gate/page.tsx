...
export default function MarketplaceGatePage() {
  ...
  async function handleSubmit() {
    ...
    setError(null);
    try {
      const res = await fetch("/api/marketplace-gate", {
        method: "POST",
  ...
  return (
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-black px-6">
      ...
      <div className="relative z-10 w-full max-w-sm rounded-lg border border-[#2A2A2E] bg-[#16161A]/90 p-8 text-center backdrop-blur-sm">
        <h1 className="text-xl font-bold text-[#E8C34A]">ZOVO Marketplace</h1>
        <p className="mt-2 text-sm text-[#9B9B95]">Page en construction. Accès réservé.</p>
